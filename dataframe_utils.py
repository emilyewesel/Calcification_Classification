...
def fill_out_missing_values_nearest(in_data=None, write_csv=False, path=None):
    ...
    # Select the visits for each subject  np.unique(in_data.loc[:, subj_id])
    for subject in np.unique(in_data.loc[:, 'subject']):
...
def text_to_codes(in_data=None, path=None, categorical = ['site', 'sex', 'cahalan_score', 'hispanic'], write_csv=False):
...
def categorical_to_onehot(in_data=None, path=None, categorical=['site', 'sex', 'cahalan_score', 'hispanic', 'race'], write_csv=False):
